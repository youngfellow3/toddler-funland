(() => {
  ...
  /* Layout sync to keep playfield inside screen above toolbar */
  ...
  function giggle(){
    beep(720,.08,'triangle',.04);
    setTimeout(()=>beep(980,.08,'triangle',.04),90);
  }

  /* Categories */
  const CATEGORIES = [
    { name:'Animals',  emojis:'🦄 🐥 🐢 🐼 🐸 🐙 🐶 🐱'.split(' ') },
    { name:'Vehicles', emojis:'🚗 🚕 🚌 🚒 🚜 🚀 🛸 🚂'.split(' ') },
    { name:'Toys',     emojis:'🧸 🪀 🪅 🎈 🎠 🎪 🪁 🧩'.split(' ') },
    { name:'Fruits',   emojis:'🍌 🍉 🍓 🍎 🍐 🍊 🍇 🥝'.split(' ') },
    { name:'Bubbles',  emojis: Array(8).fill('🫧') },
  ];
  let categoryIndex = 0;
  ...
  const SPEED_MIN = 0.04, SPEED_MAX = 0.08;
  ...
  function emojiRain(duration=5000){
    ...
    let dropTimer;
    beep(560,.08,'sine',.03);
    setTimeout(()=>beep(720,.08,'sine',.03),120);
    ...
    const spawn = () => {
      ...
      for(let i=0;i<count;i++){
        ...
        const fallMs = 3500 + Math.random()*2500;
  ...
  function fallAndSwapAfterDelay(){
    ...
    sprites.forEach(s=>{
      ...
      s.el.classList.add('main-fall');
    });

    // Hide + reset after fall
    setTimeout(()=>{
      sprites.forEach(s=>{
  ...
  addEventListener('orientationchange', () => setTimeout(()=>{ syncLayoutVars(); sprites.forEach(place); }, 300));
